package com.copypaste.coremod.client.gui;

import com.copypaste.api.util.BlockUtils;
...
import com.copypaste.blockout.controls.Button;
import com.copypaste.blockout.views.DropDownList;
import com.copypaste.coremod.CopyPaste;
import com.copypaste.coremod.management.Manager;
import com.copypaste.coremod.management.StructureName;
import com.copypaste.coremod.management.Structures;
import com.copypaste.coremod.network.messages.BuildToolPasteMessage;
import com.copypaste.coremod.network.messages.SchematicRequestMessage;
import com.copypaste.coremod.network.messages.SchematicSaveMessage;
...
import static com.copypaste.api.util.constant.WindowConstants.*;
...
public class WindowBuildTool extends AbstractWindowSkeleton
{
    ...
    private void paste(final boolean complete)
    {
        ...
        {
            CopyPaste.getNetwork().sendToServer(new BuildToolPasteMessage(
    ...
    private class SectionDropDownList implements DropDownList.DataProvider
    {
        ...
        public String getLabel(final int index)
        {
            ...
            {
                return LanguageHandler.format("com.copypaste.coremod.gui.buildtool.scans");
            }
            else if (Structures.SCHEMATICS_PREFIX.equals(name))
            {
                return LanguageHandler.format("com.copypaste.coremod.gui.buildtool.decorations");
            }
            //should be a hut
            return LanguageHandler.format("tile.copypaste.blockHut" + name + ".name");
        }
    }
    ...
    private void changeSchematic()
    {
        ...
        if (structure.isTemplateMissing() || !structure.isCorrectMD5(md5))
        {
            ...
            {
                CopyPaste.getNetwork().sendToServer(new SchematicRequestMessage(structureName.toString()));
                return;
            }
            else
            {
        ...
        if (Structures.hasMD5(structureName))
        {
            ...
            if (!Structures.hasMD5(new StructureName(serverSideName)))
            {
                ...
                if (stream != null)
                {
                    ...
                    {
                        CopyPaste.getNetwork().sendToServer(new SchematicSaveMessage(structureAsByteArray, id, 1, 1));
                    }
                    else
                    {
                        ...
                        for (int i = 1; i <= pieces; i++)
                        {
                            ...
                            CopyPaste.getNetwork().sendToServer(new SchematicSaveMessage(bytes, id, pieces, i));
                        }
                    }
                }
                else
    ...
    private void deleteClicked()
    {
        ...
        final StructureName structureName = new StructureName(schematics.get(schematicsDropDownList.getSelectedIndex()));
        confirmDeleteDialog.setTitle(LanguageHandler.format("com.copypaste.coremod.gui.structure.delete.title"));
        confirmDeleteDialog.setTextContent(LanguageHandler.format("com.copypaste.coremod.gui.structure.delete.body", structureName.toString()));
