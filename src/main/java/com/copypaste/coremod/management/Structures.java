...
import com.copypaste.api.configuration.Configurations;
import com.copypaste.api.util.Log;
import com.copypaste.api.util.MathUtils;
...
import static com.copypaste.api.util.constant.Constants.SECONDS_A_MINUTE;
import static com.copypaste.api.util.constant.Suppression.EXCEPTION_HANDLERS_SHOULD_PRESERVE_THE_ORIGINAL_EXCEPTIONS;
...
public final class Structures
{
     ...
     */
    public static final String SCHEMATICS_ASSET_PATH = "/assets/copypaste/";
    ...
    /**
     * Loads all styles saved in ["/assets/copypaste/schematics/"].
    ...
    private static void loadStyleMaps()
    {
        ...
        final File schematicsFolder = CopyPaste.proxy.getSchematicsFolder();
    ...
    private static boolean deleteCachedStructure(@NotNull final StructureName structureName)
    {
        ...
        final File structureFile = CopyPaste.proxy.getSchematicsFolder().toPath().resolve(structureName.toString() + SCHEMATIC_EXTENSION).toFile();
        if (structureFile.delete())
        {
            md5Map.remove(structureName.toString());
            return true;
